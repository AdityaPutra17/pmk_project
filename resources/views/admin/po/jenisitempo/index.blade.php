@extends('template')
@section('title', 'Jenis Item PO Management')
...
<div class="bg-slate-50 min-h-screen p-6 md:p-8 font-sans text-slate-800">
    <div class="max-w-6xl mx-auto">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Jenis Item PO Management</h1>
                <p class="text-slate-500 text-sm mt-1">Kelola jenis item PO yang digunakan pada pembelian.</p>
            </div>
        ...
        <div id="add-form-card" class="bg-white rounded-2xl shadow-sm border border-slate-200 mb-8 hidden transform transition-all duration-300 ease-in-out">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-2xl">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800">Data Jenis Item PO Baru</h3>
                </div>
            ...
            <div class="p-6">
                <form action="{{ route('jenis-item.store') }}" method="POST" class="space-y-5">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2 md:col-span-2">
                            <label for="name" class="block text-sm font-medium text-slate-700">Nama Jenis Item PO</label>
                            <input type="text" name="name" id="name" required
                                ...
                                placeholder="Elektronik, Bahan Baku, Sparepart" value="{{ old('name') }}">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2 md:col-span-2">
                            <label for="code" class="block text-sm font-medium text-slate-700">Kode Jenis Item PO</label>
                            <input type="text" name="code" id="code" required
                                class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200"
                                placeholder="JPO001, JPO002, etc." value="{{ old('code') }}">
                        </div>
                    </div>
            ...
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ $search ?? '' }}"
                        ...
                        placeholder="Cari berdasarkan nama atau kode jenis item...">
                </div>
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-all">
                    Cari
                </button>
                @if($search)
                    <a href="{{ route('jenis-item.index') }}" class="px-6 py-2.5 bg-slate-300 hover:bg-slate-400 text-slate-800 font-medium rounded-lg transition-all">
                        Reset
                    </a>
                @endif
            </form>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-slate-800">Daftar Jenis Item PO</h3>
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700">
                    {{ count($jenisItems) }} Jenis Item
                </span>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 text-slate-500 text-xs uppercase tracking-wider font-semibold">
                            ...
                            <th class="px-6 py-4 border-b border-slate-100">Nama Jenis Item PO</th>
                            <th class="px-6 py-4 border-b border-slate-100 text-right">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 text-sm text-slate-600">
                        @forelse($jenisItems as $jenisItem)
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4 font-medium text-slate-800">{{ $jenisItem->name }}</td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button onclick='openEditModal({{ $jenisItem->id }}, @json($jenisItem->code), @json($jenisItem->name) )'
                                    ...
                                    <form action="{{ route('jenis-item.destroy', $jenisItem->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            onclick="return confirm('Yakin ingin menghapus jenis item " + '{{ $jenisItem->name }}' + "?')"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <span class="text-sm font-medium">Belum ada data jenis item PO.</span>
                                </div>
                            </td>
                        </tr>
                        @endforelse
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div id="modal-panel" class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md scale-95 opacity-0" onclick="event.stopPropagation()">
                    <div class="bg-slate-50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                        <h3 class="text-lg font-semibold text-slate-800" id="modal-title">Edit Jenis Item PO</h3>
                    ...
                    <div class="px-6 py-6">
                        <form id="editForm" method="POST">
                            ...
                            <div class="space-y-4">
                                <div class="space-y-2">
                                    <label for="edit_code" class="block text-sm font-medium text-slate-700">Kode Jenis Item PO</label>
                                ...
                                <div class="space-y-2">
                                    <label for="edit_name" class="block text-sm font-medium text-slate-700">Nama Jenis Item PO</label>
...
<script>
    ...
    function openEditModal(id, code, name) {
        ...
        document.getElementById('editForm').action = `/jenis-item/${id}`;
