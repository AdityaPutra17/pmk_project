...
<div class="bg-slate-50 min-h-screen p-6 md:p-8 font-sans text-slate-800">
    <div class="max-w-6xl mx-auto">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">TOP Management</h1>
                <p class="text-slate-500 text-sm mt-1">Kelola Term of Payment dengan gaya yang konsisten seperti Supplier.</p>
            </div>
        ...
        <div id="add-form-card" class="bg-white rounded-2xl shadow-sm border border-slate-200 mb-8 hidden transform transition-all duration-300 ease-in-out">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-2xl">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800">Data TOP Baru</h3>
                </div>
            ...
            <div class="p-6">
                <form action="{{ route('top.store') }}" method="POST" class="space-y-5">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label for="code" class="block text-sm font-medium text-slate-700">Kode TOP</label>
                            <input type="text" id="code" value="{{ $newTopCode }}" readonly
                                class="block w-full px-4 py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 cursor-not-allowed">
                        </div>

                        <div class="space-y-2 md:col-span-2">
                            <label for="description" class="block text-sm font-medium text-slate-700">Deskripsi TOP</label>
                            <input type="text" name="description" id="description" required
                                ...
                                placeholder="Jatuh tempo 30 hari" value="{{ old('description') }}">
                        </div>
                    </div>
            ...
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ $search ?? '' }}"
                        ...
                        placeholder="Cari berdasarkan kode TOP atau deskripsi...">
                </div>
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-all">
                    Cari
                </button>
                @if($search)
                    <a href="{{ route('top.index') }}" class="px-6 py-2.5 bg-slate-300 hover:bg-slate-400 text-slate-800 font-medium rounded-lg transition-all">
                        Reset
                    </a>
                @endif
            </form>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-slate-800">Daftar TOP</h3>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 text-slate-500 text-xs uppercase tracking-wider font-semibold">
                            ...
                            <th class="px-6 py-4 border-b border-slate-100">Kode TOP</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm text-slate-600">
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4 font-medium text-slate-800">{{ $top->code }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $top->description }}</td>
                            <td class="px-6 py-4 text-right">
                                <form action="{{ route('top.destroy', $top->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        onclick="return confirm('Yakin ingin menghapus TOP ini?')"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <span class="text-sm font-medium">Belum ada data TOP.</span>
                                </div>
                            </td>
                        </tr>
                        @endforelse
