    ...
    <div class="max-w-6xl mx-auto">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
            <div>
                ...
                <p class="text-slate-500 text-sm mt-1">Kelola item PO dengan tampilan yang sama seperti daftar supplier.</p>
            </div>
            ...
            <div class="p-6">
                <form action="{{ route('item-po.store') }}" method="POST" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label for="item_code" class="block text-sm font-medium text-slate-700">Kode Item PO</label>
                            <input type="text" id="item_code" value="{{ $newItemCode }}" readonly
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <select name="id_jenis_item_po" id="id_jenis_item_po" required
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="3" required
                                ...
                                placeholder="Deskripsi item PO">{{ old('description') }}</textarea>
                        </div>
                    </div>
            ...
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ $search ?? '' }}"
                        ...
                        placeholder="Cari berdasarkan kode item atau deskripsi...">
                </div>
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-all">
                    Cari
                </button>
                @if($search)
                    <a href="{{ route('item-po.index') }}" class="px-6 py-2.5 bg-slate-300 hover:bg-slate-400 text-slate-800 font-medium rounded-lg transition-all">
                        Reset
                    </a>
                @endif
            </form>
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <form action="{{ route('item-po.destroy', $itemPO->id) }}" method="POST" class="inline">
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        onclick="return confirm('Yakin ingin menghapus item PO ini?')"
