            ...
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ $search ?? '' }}"
                        ...
                        placeholder="Cari berdasarkan nama customer, alamat, atau nomor telepon...">
                </div>
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-all">
                    Cari
                </button>
                @if($search)
                    <a href="{{ route('customers.index') }}" class="px-6 py-2.5 bg-slate-300 hover:bg-slate-400 text-slate-800 font-medium rounded-lg transition-all">
                        Reset
                    </a>
                @endif
            </form>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center">
                <div class="text-sm text-slate-600">
                    Menampilkan <span class="font-medium">{{ $customers->firstItem() ?? 0 }}</span> hingga <span class="font-medium">{{ $customers->lastItem() ?? 0 }}</span> dari <span class="font-medium">{{ $customers->total() }}</span> data
                </div>
                <div>
                    {{ $customers->links() }}
                </div>
