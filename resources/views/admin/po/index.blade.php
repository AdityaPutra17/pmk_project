...
<div class="bg-slate-50 min-h-screen p-6 md:p-8 font-sans text-slate-800">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Purchase Order Management</h1>
                <p class="text-slate-500 text-sm mt-1">Buat dan kelola Purchase Order dengan tampilan serupa Supplier PO.</p>
            </div>
        ...
        <div id="add-form-card" class="bg-white rounded-2xl shadow-sm border border-slate-200 mb-8 hidden transform transition-all duration-300 ease-in-out">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-2xl">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800">Form Purchase Order Baru</h3>
                </div>
            ...
            <div class="p-6">
                <form action="{{ route('po.store') }}" method="POST" class="space-y-5">
                    @csrf
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            <label for="po_date" class="block text-sm font-medium text-slate-700">Tanggal PO</label>
                            <input type="date" name="po_date" id="po_date" required value="{{ old('po_date') }}"
                        ...
                        <div class="space-y-2">
                            <label for="delivery_date" class="block text-sm font-medium text-slate-700">Tanggal Delivery</label>
                            <input type="date" name="delivery_date" id="delivery_date" value="{{ old('delivery_date') }}"
                                class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200">
                        </div>

                        <div class="space-y-2">
                            <label for="top_id" class="block text-sm font-medium text-slate-700">TOP</label>
                            <select name="top_id" id="top_id"
                                class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200">
                                <option value="">Pilih TOP</option>
                                @foreach($tops ?? [] as $top)
                                <option value="{{ $top->id }}" {{ old('top_id') == $top->id ? 'selected' : '' }}>{{ $top->description }}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="space-y-2">
                            <label for="franco_id" class="block text-sm font-medium text-slate-700">Franco</label>
                            <select name="franco_id" id="franco_id"
                                class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200">
                                <option value="">Pilih Franco</option>
                                @foreach($francos ?? [] as $franco)
                                <option value="{{ $franco->id }}" {{ old('franco_id') == $franco->id ? 'selected' : '' }}>{{ $franco->name }}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="space-y-2">
                            <label for="ppn" class="block text-sm font-medium text-slate-700">PPN</label>
                            <select name="ppn" id="ppn" required
                                class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200">
                                ...
                                <table class="w-full text-sm" id="itemTable">
                                    ...
                                    <thead>
                                        <tr class="bg-slate-100 text-slate-700">
                                            ...
                                            <th class="px-4 py-3 text-left w-40">Total</th>
        ...
        <div class="mb-8 bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                ...
                @if($search)
                    <a href="{{ route('po.index') }}" class="px-6 py-2.5 bg-slate-300 hover:bg-slate-400 text-slate-800 font-medium rounded-lg transition-all">
                        Reset
                    </a>
                @endif
            </form>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-slate-800">Daftar Purchase Order</h3>
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700">
                    {{ count($purchaseOrders ?? []) }} PO
                </span>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 text-slate-500 text-xs uppercase tracking-wider font-semibold">
                            ...
                            <th class="px-6 py-4 border-b border-slate-100">Nomor PO</th>
                            <th class="px-6 py-4 border-b border-slate-100">Supplier</th>
                            <th class="px-6 py-4 border-b border-slate-100">Customer</th>
                            <th class="px-6 py-4 border-b border-slate-100">TOP</th>
                            <th class="px-6 py-4 border-b border-slate-100">Franco</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm text-slate-600">
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="px-6 py-4">{{ $po->customer->name ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $po->top->description ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $po->franco->name ?? '-' }}</td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('po.print',$po->id) }}"
                                target="_blank"
                                class="p-2 bg-blue-50 text-blue-600 rounded-lg">
                                    Print
                                </a>
                                
                                <form action="{{ route('po.destroy', $po->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <span class="text-sm font-medium">Belum ada data Purchase Order.</span>
                                </div>
                            </td>
                        </tr>
                        @endforelse
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center">
                <div class="text-sm text-slate-600">
                    Menampilkan <span class="font-medium">{{ $purchaseOrders->firstItem() ?? 0 }}</span> hingga <span class="font-medium">{{ $purchaseOrders->lastItem() ?? 0 }}</span> dari <span class="font-medium">{{ $purchaseOrders->total() }}</span> data
                </div>
                <div>
    ...
    <tr class="hover:bg-slate-50">
        ...
        <td class="p-3">
            <select
                ...
                class="item-select w-full rounded-lg border border-slate-300 px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
        ...
        <td class="p-3 text-center">
            <button
                ...
                class="inline-flex items-center justify-center px-3 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition">
