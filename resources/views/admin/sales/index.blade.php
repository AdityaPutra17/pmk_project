...
<div class="bg-slate-50 min-h-screen p-6 md:p-8 font-sans text-slate-800">
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Sales Management</h1>
                <p class="text-slate-500 text-sm mt-1">Kelola data salesman dan wilayah operasional.</p>
            </div>
        ...
        <div id="add-form-card" class="bg-white rounded-2xl shadow-sm border border-slate-200 mb-8 hidden transform transition-all duration-300 ease-in-out">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-2xl">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-lg font-semibold text-slate-800">Data Sales Baru</h3>
                </div>
            ...
            <div class="p-6">
                <form action="{{ route('sales.store') }}" method="POST" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Input Name -->
                        <div class="space-y-2">
                            <label for="name" class="block text-sm font-medium text-slate-700">Nama Sales</label>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            <label for="area_id" class="block text-sm font-medium text-slate-700">Area Tugas</label>
                            <div class="relative group">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-400 group-focus-within:text-indigo-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                <select name="area_id" id="area_id" required
                                    class="block w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 appearance-none">
                                    <option value="">-- Pilih Area --</option>
                                    @foreach($areas as $area)
                                        <option value="{{ $area->id }}">{{ $area->name }} ({{ $area->kd_area }})</option>
                                    @endforeach
                                </select>
                                <!-- Dropdown Arrow Icon -->
            ...
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ $search ?? '' }}"
                        ...
                        placeholder="Cari berdasarkan nama sales, kode, atau nomor telepon...">
                </div>
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-all">
                    Cari
                </button>
                @if($search)
                    <a href="{{ route('sales.index') }}" class="px-6 py-2.5 bg-slate-300 hover:bg-slate-400 text-slate-800 font-medium rounded-lg transition-all">
                        Reset
                    </a>
                @endif
            </form>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-slate-800">Daftar Sales</h3>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 text-slate-500 text-xs uppercase tracking-wider font-semibold">
                            ...
                            <th class="px-6 py-4 border-b border-slate-100">Nama Sales</th>
                            <th class="px-6 py-4 border-b border-slate-100">Kode</th>
                            <th class="px-6 py-4 border-b border-slate-100">Area</th>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('sales.destroy', $s->id) }}" method="POST" class="inline">
                                        ...
                                        @method('DELETE')
                                        <button type="submit" 
                                            onclick="return confirm('Yakin ingin menghapus sales {{ $s->name }}?')"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <span class="text-sm font-medium">Belum ada data sales.</span>
                                </div>
                            </td>
                        </tr>
                        @endforelse
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div id="modal-panel" class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md scale-95 opacity-0" onclick="event.stopPropagation()">
                    ...
                    <div class="bg-slate-50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                        <h3 class="text-lg font-semibold text-slate-800" id="modal-title">Edit Sales</h3>
                    ...
                    <div class="px-6 py-6">
                        <form id="editForm" method="POST">
                            ...
                            <div class="space-y-4">
                                <!-- Nama Sales -->
                                <div class="space-y-2">
                                    <label for="edit_name" class="block text-sm font-medium text-slate-700">Nama Sales</label>
                                ...
                                <!-- Kode Sales (Readonly) -->
                                <div class="space-y-2">
                                    <label for="edit_kd_sales" class="block text-sm font-medium text-slate-700">Kode Sales</label>
                                    <input type="text" name="kd_sales" id="edit_kd_sales" readonly 
                                ...
                                <!-- Area -->
                                <div class="space-y-2">
                                    <label for="edit_area_id" class="block text-sm font-medium text-slate-700">Area Tugas</label>
